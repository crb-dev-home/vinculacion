...
function RotatedText($x, $y, $txt, $angle)
{
	...
	$pdf->Cell(0,4,utf8_decode('Asunto: CARTA DE POSTULACIÃ“N PARA LA BECA'),0,1,'R');
	$pdf->Cell(0,4,utf8_decode('DE SERVICIO SOCIAL '.date('Y').' PARA ALUMNOS INSCRITOS'),0,1,'R');
	$pdf->Ln(7);

    $pdf->SetFont('Arial','B',10);
    $pdf->Cell(0,4,utf8_decode($filaCoordinador['valor']).',',0,1,'L');
//'MTRO. JOSÃ‰ ERNESTO MEDINA AGUILAR,'),0,1,'L');
	...
	$pdf->MultiCell(177,6,'Por medio de la presente y atendiendo los requisitos y procedimientos de la convocatoria de Becas de Servicio Social '.date('Y').', nos permitimos postular al alumno inscrito que cumple con los créditos requeridos para realizar su servicio social:',0,'J');
    $pdf->Ln(2);
    $pdf->SetFont('Arial','B',10);
    $pdf->MultiCell(177,6,'- Nombre completo del alumno inscrito: '.$fila['nomAlumno'].' '.$fila['apPaterno'].' '.$fila['apMaterno'],0,'J');
    $pdf->MultiCell(177,6,'- Nombre de la institución receptora en que ha sido aceptado para realizar el servicio social: '.$fila['nomDependencia'],0,'J');
    $pdf->MultiCell(177,6,'- Nombre del Programa de Servicio Social autorizado por la IPES: '.$fila['programa'],0,'J');
    $pdf->MultiCell(177,6,'- Periodo en el que se realizará el servicio social (mínimo 6 meses a partir del '.date('d',$fechaInicioBeca).' de '.$meses[date('m',$fechaInicioBeca)-1].' del '.date('Y',$fechaInicioBeca).' al '.date('d',$fechaTerminoBeca).' de '.$meses[date('m',$fechaTerminoBeca)-1].' del '.date('Y',$fechaTerminoBeca).'):',0,'J');
    ...
    $pdf->MultiCell(177,6,'- Número de horas a prestar en Servicio Social (mínimo 480 hrs.): '.$fila['horas'].' HRS',0,'L');
    $pdf->Ln(2);
    $pdf->SetFont('Arial','',10);
    $pdf->MultiCell(177,6,'Por lo anteriormente expresado, se confirma que el solicitante ha sido autorizado para el desempeño del servicio social en un programa reconocido por esta institución.',0,'J');
    $pdf->SetFont('Arial','B',10);
    $pdf->SetXY(20, 200);
