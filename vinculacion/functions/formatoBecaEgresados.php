...
function RotatedText($x, $y, $txt, $angle)
{
	...
	$pdf->Cell(0,4,utf8_decode('DE SERVICIO SOCIAL '.date('Y').' PARA EGRESADOS'),0,1,'R');
	$pdf->Ln(7);
...
//  $pdf->Cell(0,4,utf8_decode('MTRO. JOSÃ‰ ERNESTO MEDINA AGUILAR,'),0,1,'L');
	...
	$pdf->MultiCell(177,6,'Por medio de la presente y atendiendo los requisitos y procedimientos de la convocatoria de Becas de Servicio Social '.date('Y').', nos permitimos postular al egresado que cumple con el 100% de los créditos de su programa:',0,'J');
    $pdf->Ln(2);
    $pdf->SetFont('Arial','B',10);
    $pdf->MultiCell(177,6,'- Nombre completo del egresado: '.$fila['nomAlumno'].' '.$fila['apPaterno'].' '.$fila['apMaterno'],0,'J');
    ...
    $pdf->Ln(2);
    $pdf->SetFont('Arial','',10);
    $pdf->MultiCell(177,6,'Asimismo, se confirma que el solicitante ha sido autorizado para el desempeño del servicio social en un programa reconocido por esta institución.',0,'J');
    $pdf->SetFont('Arial','B',10);
    $pdf->SetXY(20, 195);
